...
- Sleep / Timer（実装済み）
- Signal / Channel（実装済み）
- 並行Activity実行 / Future（実装済み）
- ワークフローキャンセル（実装済み）
- Activityハートビート（実装済み）
- Child Workflow（実装済み: StartChildWorkflowコマンド、親子関係管理、子完了/失敗の親への伝搬）
...
- Continue-as-New（実装済み: ContinueAsNewコマンド、旧WFをCONTINUED_AS_NEWにして新WF自動作成、WorkflowType/TaskQueue/CronSchedule引き継ぎ）
- Cron / スケジュール実行（実装済み: StartWorkflowでcron_schedule指定、CompleteWorkflow時にContinue-as-Newで自動再起動、FailWorkflow時は再起動しない）
- Namespace（実装済み: namespacesテーブル、全テーブルにnamespaceカラム追加、全APIにnamespaceパラメータ追加、デフォルトnamespace "default"）
- SideEffect（実装済み: RecordSideEffectコマンド、SideEffectRecordedイベント記録。リプレイ値復元はSDK側の責務）
- Query（実装済み: QueryWorkflow API、RespondQueryTask API、workflow_queriesテーブル、PollWorkflowTaskにpending_queries含む）
- Update（Phase 4）
...
- タイムアウトサポート:
  ...
  - ScheduleToCloseTimeout（実装済み）: Activityスケジュールから完了までの総時間制限。リトライを跨いで有効
  - ScheduleToStartTimeout（実装済み）: Activityスケジュールからワーカー取得までの時間制限。PENDINGタスクのみ対象、Poll時にクリア
...
| SignalWorkflow | 実行中のワークフローにシグナル送信 |
| CancelWorkflow | ワークフローのグレースフルキャンセル通知 |
| ListWorkflows | ワークフロー一覧取得（フィルタ・ページネーション対応） |
| QueryWorkflow | 実行中ワークフローの状態問い合わせ |
...
| RecordActivityHeartbeat | Activityハートビート報告 |

### 3.2 オプション機能（段階的に追加）

Phase 2:

- Timer / Sleep（実装済み: StartTimer/CancelTimerコマンド、TimerPollerバックグラウンドワーカー）
- Signal / Channel（実装済み: SignalWorkflow API、WorkflowSignaledイベント記録 + WorkflowTask投入）
- 並行Activity実行（実装済み: 1回のCompleteWorkflowTaskで複数ScheduleActivityコマンドを同時送信、各Activityが独立にPoll・Complete可能）
- ワークフローキャンセル（実装済み: CancelWorkflow API、WorkflowCancelRequestedイベント記録 + WorkflowTask投入、ワークフロー状態は変更しないgraceful方式）
- Activityハートビート（実装済み: RecordActivityHeartbeat API、heartbeat_at/heartbeat_timeout管理、BackgroundWorkerによるタイムアウト検知）
- ScheduleToCloseTimeout / ScheduleToStartTimeout（実装済み）
- 構造化ログ整備（実装済み: slog統一）
- CLIツール（実装済み: dandori-cli、cobra製、start/describe/terminate/cancel/signal/list/historyサブコマンド）
- ListWorkflows API（実装済み: cursor-based pagination、status/type/queueフィルタ）
- LISTEN/NOTIFYによるタスク即時通知
- sticky execution（ワーカー側イベントキャッシュ）

Phase 3:

- Child Workflow（実装済み: StartChildWorkflowコマンド、親子関係管理、子完了/失敗の親へのイベント伝搬、E2Eテスト）
...
- SideEffect（実装済み: RecordSideEffectコマンド → SideEffectRecordedイベント記録のみ。WorkflowTask未生成。リプレイ値復元はSDK側の責務）
...
- Namespace（実装済み: namespacesテーブル、namespaceカラム追加、全APIにnamespace対応、CLIに--namespaceフラグ、デフォルト "default"）
...
- ヘルスチェック（実装済み: gRPC Health v1 + HTTP `/healthz`）
- Namespace（実装済み: マルチテナント分離、namespacesテーブル、全テーブルにnamespaceカラム）
...
- Cronパーサー: github.com/robfig/cron/v3
- ログ: 標準ライブラリ log/slog
