...
- [ ] internal/domain/command.go — `Command` 構造体に `Metadata map[string]string` フィールド追加
- [ ] api/v1/types.proto — `CompleteWorkflowTaskRequest` に `map<string, string> metadata = 3` フィールド追加。protoc再生成
- [ ] internal/adapter/grpc/handler.go — `CompleteWorkflowTask` でmetadataフィールドをdomain.Commandに伝搬
- [ ] internal/engine/engine.go — `CompleteWorkflowTask` でmetadataをイベントのevent_dataに含めて保存
- [ ] test/e2e/saga_test.go（新規）— Saga補償パターンのE2Eテスト
...
- [ ] metadataフィールドがCompleteWorkflowTaskRequest → イベントevent_data に正しく保存される
- [ ] Saga補償パターン（正常系: 逆順補償実行）がE2Eテストで検証される
- [ ] ContinueWithErrorパターン（補償失敗時の続行）がE2Eテストで検証される
- [ ] metadataがGetWorkflowHistoryで取得可能であることがE2Eテストで検証される
- [ ] `go test -v -race ./...` — 全テスト通過
- [ ] `go vet ./...` — クリーン
